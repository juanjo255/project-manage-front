    ...
    <li onClick={() => deleteToken()}>
      <NavLink to="/auth/login" className="sidebar-route">
        <div className="flex items-center text-red-600">
          ...
          <span className="font-bold text-white text-sm  ml-2">Log out</span>
        </div>
      </NavLink>
    </li>
  );
...
const Sidebar = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row flex-no-wrap h-screen absolute md:static md:border-black md:border-solid md:border-r-4">
      {/* Sidebar starts */}
      <div className="sidebar bg-gray-800 hidden md:flex">
...
const ResponsiveSidebar = ({ setOpen }) => {
  return (
    <div
      className={`h-screen md:hidden bg-gray-800 overflow-auto rounded-r-sm`}
      id="mobile-nav"
  ...
  return (
    <li>
      <NavLink
        ...
        className={({ isActive }) =>
          isActive
            ? "sidebar-route border-t-2 border-b-2 font-extrabold border-green-400 text-green-400"
            : "sidebar-route font-bold text-white hover:text-black hover:bg-green-500"
        }
      >
